...
class Skysweeper():
    ...
    def loseGame(self):
        ...
        message = tkMessageBox.askyesno(title="Game Over!", message= "You were on Level: "+str(count)+" and you lost in "+ "%s seconds." %(time.clock() - start_time)+" Try again?",parent = window)
        try:
            for x in range(2):
                loseSound.play()
    ...
    def winGame(self):
        ...
        if len(reg_constellations) != 0:
            ...
            for x in self.tiles:
                self.clearTiles(self.tiles[x][3])
        window2 = tkMessageBox.askyesno(title= "You Beat Level: "+str(count)+"!", message= "Your time was "+ " %s seconds." % (time.clock() - start_time)+" Continue?")
        try:
            for x in range(2):
                winSound.play()
                GPIO.output(smiley, True)#turns the smiley leds on
                sleep(0.5)
                GPIO.output(smiley, False)#turns the smiley leds off
...
def pick():
    ...
    temp = [(i) for i in reg_constellations]
    if len(temp) >= 1:
        random.shuffle(temp)
        picked = temp[0]
